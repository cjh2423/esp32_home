...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    // Create the default WiFi AP/STA interfaces
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t done_html = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            std::string html = done_html_start;
            std::string project_url = this_->sta_ip_address_.empty() ? "IP unavailable" : "http://" + this_->sta_ip_address_;

            size_t pos = html.find("{{PROJECT_WEB_URL}}");
            if (pos != std::string::npos) {
                html.replace(pos, strlen("{{PROJECT_WEB_URL}}"), project_url);
            }
...
void WifiConfigurationAp::IpEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == IP_EVENT_STA_GOT_IP) {
        ...
        self->sta_ip_address_ = ip_address;
