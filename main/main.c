    ...
    while (1) {
        ...
        if (mq2_ok) {
            if (mq2_read(MQ2_ADC_CHANNEL, &smoke_val) == ESP_OK) {
                g_sensor_data.smoke = smoke_val;
            }
        }
        
        // --- 2. 业务逻辑处理 ---
        app_control_process(&g_sensor_data);
...
void app_main(void)
{
    ...
    // Fan Init 不再需要 RELAY_HUMIDIFIER_GPIO 参数，稍后修改 fan.c/h
    // 先暂时传一个无效引脚 -1 或者保留原参数如果还没改 fan 的话。
    // 最好现在就去改 fan.c 将其解耦。
    // 但为了步进一致，假设 fan_init 签名还没变，我们传 -1
    ESP_ERROR_CHECK(fan_init(FAN_GPIO, FAN_PWM_CHANNEL, -1)); 
    ESP_ERROR_CHECK(buzzer_init(BUZZER_GPIO));
    
    // 舵机初始化，后两个参数由驱动内部忽略
    ESP_ERROR_CHECK(motor_init(SERVO_GPIO, 0, 0));
    
    // 开机自检
    buzzer_beep(BUZZER_GPIO, 100);
    
    // 3. 初始化网络 (Network Layer)
    wifi_init_sta(WIFI_SSID, WIFI_PASS, on_wifi_connected, on_wifi_disconnected);
    http_server_start(&g_sensor_data);
