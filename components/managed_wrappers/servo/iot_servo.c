 ...
 *
 * Modified: 强制使用 LEDC_APB_CLK 避免与其他 LEDC 模块时钟源冲突
 */
...
// 修改：使用 14-bit 分辨率以支持 50Hz 低频 + APB_CLK (80MHz)
// 80MHz / 2^14 / 50Hz = 97.65，分频系数在有效范围内
...
esp_err_t iot_servo_init(ledc_mode_t speed_mode, const servo_config_t *config)
{
    ...
    ledc_timer_config_t ledc_timer = {
        .clk_cfg = LEDC_APB_CLK,  // 修改：强制使用 APB_CLK，与 LED/Fan 一致，避免时钟冲突
